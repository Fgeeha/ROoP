...
- `doc/offline-models.md` — getting the models onto a machine that cannot reach `registry.ollama.ai` at all. Four routes, cheapest first: pull on the host and copy into the `ollama_data` volume (works when the corporate root certificate is in the system store, so the browser and the host succeed while the container fails); a donor machine with a dedicated model directory; a selective transfer that reads each manifest and copies only the blobs it references; and importing a GGUF file with `ollama create`. Also covers moving the images with `docker save`, and the detail that trips people up: `docker compose up` starts the `ollama-pull` sidecar, which tries the registry, so an offline start has to name the services explicitly. The README previously described only a whole-volume `tar`, which is 50 GB when the donor machine holds a dozen models.
